...
import { Scan, Clock, CheckCircle, AlertTriangle, Plus } from "lucide-react";
...
function statusVariant(status: string) {
  if (status === "SCANNED") return "info" as const;
...
export default function RadiologyDashboard() {
  const { data, isLoading, error, refetch, isFetching } = useQuery<
    RadiologyRequest[]
  >({
    queryKey: ["radiology_dashboard_requests"],
    queryFn: async () => {
      const res = await fetch("/api/v1/radiology/requests");
      if (!res.ok) {
        const json = await res.json().catch(() => null);
        throw new Error(
          json?.error?.message ?? "Failed to load radiology requests"
        );
      }
  ...
  const pendingScans =
    data?.filter((r) => r.status === "REQUESTED").length ?? 0;
  const awaitingReport =
    data?.filter((r) => r.status === "SCANNED").length ?? 0;
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Scan requests, imaging, and reporting queue.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {isLoading ? (
          ...
          <>
            <StatCard title="Pending Scans" value={pendingScans} icon={Clock} />
            <StatCard
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-base font-medium text-destructive">
                  Failed to load radiology requests
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
