...
import { ClipboardList, HeartPulse, RefreshCw } from "lucide-react";
...
interface QueueVisit {
  ...
  appointment?: { reason?: string } | null;
}
...
export default function NurseTriageQueue() {
  const { data, isLoading, error, refetch, isFetching } = useQuery<{ total: number; visits: QueueVisit[] }>({
    queryKey: ["nurse_triage_queue"],
    queryFn: async () => {
      // IN_PROGRESS visits without vitals recorded = waiting for triage
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Patients awaiting vitals assessment.
            {data && (
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
              <ClipboardList className="w-12 h-12 text-destructive/40 mb-2" />
                ...
                <tbody className="[&_tr:last-child]:border-0">
                  {queue.map((visit) => {
                    ...
                    return (
                      ...
                      >
                        <td className="p-6 align-middle font-medium text-muted-foreground text-sm">
